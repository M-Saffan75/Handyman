...
import { categories, reviews } from '../../Backend/data';
...
const Categories_Details_Here = ({ route }) => {

    const { subId } = route.params
    const navigation = useNavigation();
    const theme = useContext(themeContext);
    const [categorie_data, setCategorie_data] = useState(null)
    const selectedItem = categories.find(category =>
        category.categore_service.find(subcategory => subcategory.subId === subId)
    );
    
    useEffect(() => {
        if (selectedItem) {
            const selectedSubcategory = selectedItem.categore_service.find(
                subcategory => subcategory.subId === subId
            );
            setCategorie_data(selectedSubcategory);
    ...
    return (
        <>
            ...
            <SafeAreaView>
                ...
                <TouchableOpacity style={[styles.book_btn, { backgroundColor: COLOURS.blue }]} activeOpacity={0.9} onPress={() => navigation.goBack()}>
...
export default Categories_Details_Here