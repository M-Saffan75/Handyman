...
const Online_Handy_Details = ({ route }) => {

    const { id } = route.params
    const navigation = useNavigation();
    const theme = useContext(themeContext);
    const [onlinehandy, setOnlinehandy] = useState(null)
    const selectedItem = online_handymans.find(item => item.id === id);

    useEffect(() => {
        setOnlinehandy(selectedItem);
    }, [selectedItem]);


    const inputFields = [
        { label: 'name', placeholder: 'Name', icon: require('../../../Bold/user.png'), defaultValue: onlinehandy?.provider_name },
        { label: 'lastname', placeholder: 'Lastname', icon: require('../../../Bold/shield.png'), defaultValue: onlinehandy?.lastname },
        { label: 'username', placeholder: 'Username', icon: require('../../../Bold/user.png'), keyboardType: 'username', defaultValue: onlinehandy?.username },
        { label: 'email', placeholder: 'Email', icon: require('../../../Bold/envelope.png'), keyboardType: 'email-address', defaultValue: onlinehandy?.email },
        { label: 'contact', placeholder: 'Contact', icon: require('../../../Bold/phone.png'), keyboardType: 'numeric', defaultValue: onlinehandy?.phone },
        { label: 'designation', placeholder: 'Designation', defaultValue: onlinehandy?.designation },
        { label: 'password', placeholder: 'Password', icon: require('../../../Bold/eye-slash.png'), secureTextEntry: true, defaultValue: onlinehandy?.password },
    ];
                    ...
                    <View>
                        ...
                        <View style={styles.bg_area_book}>
                            <View style={styles.arrow_area}>
                                ...
                                <Text style={[styles.acc_name, { color: COLOURS.white }]}>update</Text>
                            </View>
                        ...
                        <TouchableWithoutFeedback onPressIn={() => setMenuVisible(false)}>
                            <View style={styles.profile_handy_bg}>
                                <Image source={onlinehandy?.provider_profile} style={styles.profile_handy} />
                            </View>
                        ...
                        <View>
                            <View style={styles.btn_center}>
                                <TouchableOpacity style={styles.btn} activeOpacity={0.8}>
                                    <Text style={styles.btn_text}>update changes</Text>
                                </TouchableOpacity>
                            </View>
                        </View>
...
export default Online_Handy_Details
